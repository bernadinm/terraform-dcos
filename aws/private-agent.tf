...
resource "null_resource" "agent" {
  ...
  provisioner "remote-exec" {
    inline = [
     ...
     "sudo echo 'MESOS_CREDENTIAL=/run/dcos/etc/credentials' | sudo tee /run/dcos/etc/mesos-slave",
     "sudo echo '{\"principal\":\"principal1\",\"secret\":\"secret1\"}' | sudo tee /run/dcos/etc/credentials"
    ]
  }
